...
int main (int argc, char **argv) {
    ...
    case 4:
        status = exist(0);
        if (status != 1) perror("Error: the file exists but it was evaluated as non-existant\n");
        status = exist(1);
        if (status != 0) perror("Error: the file does not exists but it was evaluated as existant\n");
        break;
    case 5:
